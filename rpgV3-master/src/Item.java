...
public class Item {
    ...
    public void prosesAksi(int pil) {
        ...
        if (pil==1) {
            ...
            if(getNama().equals("Pedang")){
                System.out.println(" dengan damage "+objGameInfo.getObjPlayer().getDamage());
            }
        } else  if (pil==2) {  //bisa ambil atau buang
            if (objRuangan==null) {
               //dipegang player, buang ke ruangan
                if(getNama().equals("Pedang")){
                    System.out.println("Pedang jangan dibuang!!");
                    System.out.println("Karena akan digunakan untuk melawan Zombie!");
                }else{
                    dibuang();
                }
            } else {
